...
AddSubComponents). See CLAUDE.md gotchas.
"""
...
from stp2aas.extract.p21_header import parse_header
...
def extract(path: str) -> StepDocument:
    ...
    top_nodes = [
        _walk(shape_tool, roots.Value(i), pmi_entries, color_tool)
    ...
    return StepDocument(
        ...
        xcaf_doc=doc,  # keeps the labels behind color_tool alive
    )
...
def load_geometry(path: str):
    ...
    if roots.Length() == 1:
        ...
        shape = comp
    return shape, _physical_props(shape), _bounding_box(shape), pmi
...
def _walk(shape_tool, label: TDF_Label, pmi_entries: set[str], color_tool) -> PartNode:
    ...
    if shape_tool.IsAssembly(label):
        ...
        for i in range(1, comps.Length() + 1):
            node.children.append(
                _walk_component(shape_tool, comps.Value(i), pmi_entries, color_tool)
            )
        return node
    return _leaf(shape_tool, label, pmi_entries, color_tool)


def _walk_component(
    shape_tool, comp_label: TDF_Label, pmi_entries: set[str], color_tool
) -> PartNode:
    ...
    if shape_tool.GetReferredShape(comp_label, ref):
        node = _walk(shape_tool, ref, pmi_entries, color_tool)  # recurse into prototype
    else:
        node = _leaf(shape_tool, comp_label, pmi_entries, color_tool)
...
def _leaf(shape_tool, label: TDF_Label, pmi_entries: set[str], color_tool) -> PartNode:
...
def _shape_color(color_tool, shape_tool, label, shape) -> tuple[float, float, float] | None:
...
def _physical_props(shape) -> PhysicalProps:
    ...
    try:
        ...
        area = sp.Mass()
    except Exception:  # noqa: BLE001 — degrade gracefully on pathological shapes
        pass
