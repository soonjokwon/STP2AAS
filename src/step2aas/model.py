...
class PhysicalProps:
    ...
    density_g_cm3: float | None = None  # S8
    mass_kg: float | None = None  # B7 (derived: density × volume)
...
class PartNode:
    ...
    # multiple BOM occurrences refer to that asset (design decision D2).
    ref_key: str = ""
    ...
    # S9: representative RGB (0..1) from XCAF, kept when re-exporting a split part.
...
class StepDocument:
    ...
    header: P21Header
    source_path: str
    file_hash: str  # input identity fingerprint for deterministic AAS IDs and D5
    # S9: source XCAF colour tool (+ its document, which must stay alive).
    color_tool: object | None = None
    xcaf_doc: object | None = None
    # D5: XML v2 fingerprints the consumed dependency set; P21 IDs stay unchanged.
    source_manifest: list[dict[str, str]] = field(default_factory=list)
    identity_scheme: str = "p21-sha256-v1"
    legacy_file_hash: str | None = None
