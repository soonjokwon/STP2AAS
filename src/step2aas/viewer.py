...
.html (a small hand-written WebGL renderer, no third-party JS).
    ...
    python -m step2aas.viewer --input out/suspension.aasx [-o out.html] [--open] [--no-3d]
"""
...
def _elem_disp(obj) -> str | None:
    ...
    if dn:
        try:
            ...
            logger.debug("Display name unavailable: %s", exc)
    return None
    ...
    for target in (face, shape):
        for kind in (XCAFDoc_ColorSurf, XCAFDoc_ColorGen):
            if color_tool.GetColor(target, kind, c):
                # XCAF stores linear RGB; the WebGL framebuffer is display-referred
                # sRGB. Convert so CAD yellow does not render as olive.
                to_s = Quantity_Color.Convert_LinearRGB_To_sRGB
                return [
                    max(0, min(255, round(to_s(c.Red()) * 255))),
                    max(0, min(255, round(to_s(c.Green()) * 255))),
                    max(0, min(255, round(to_s(c.Blue()) * 255))),
                ]
    return fallback
...
def _edge_segments(shape, deflection: float) -> list[float]:
    ...
    while exp.More():
        ...
        try:
            ...
            logger.warning("Skipping an edge that could not be discretized: %s", exc)
            continue
    return out
...
def build_meshes(store, files, enable: bool) -> dict:
    ...
    def tess_shell(shell) -> None:
        ...
        if not stp_name or not stp_name.lower().endswith(_STEP_EXT):
            return
        # Many shells can share the same embedded STEP supplementary file.
        # Tessellating that geometry repeatedly is expensive and can trigger
        # memory/resource pressure; cache by supplementary STEP name.
        if stp_name in file_raw:
            m = file_raw[stp_name]
        else:
            try:
            ...
            file_raw[stp_name] = m
        if m is None:
            return  # neither faces nor edges — nothing to show (or tessellation failed)
        ...
        if name.endswith("__scene.json"):
            ...
            if data:
                try:
                    ...
                    logger.warning("Skipping invalid scene %s: %s", name, exc)
    return scenes
...
def build_html(aasx_path, store, files, meshes) -> str:
    ...
    # A4: occurrence Entity.globalAssetId links each child to its prototype AAS.
    # EntryNode identifies the owning asset and must not mark that asset as a child.
    # Retain the old external SameAs target for packages produced before this fix.
    child_assets: set[str] = set()
    for sm in submodels:
        if sm.id_short != "HierarchicalStructures":
            continue
        entries = [el for el in sm.submodel_element if isinstance(el, model.Entity)]
        for entry in entries:
            for el in _walk(entry):
                if el is not entry and isinstance(el, model.Entity) and el.global_asset_id:
                    child_assets.add(el.global_asset_id)
                elif (
                    isinstance(el, model.RelationshipElement)
                    and (el.id_short or "").startswith("SameAs")
                    and isinstance(el.second, model.ExternalReference)
                ):
                    child_assets.add(el.second.key[-1].value)

    def level(s):  # 0 root assembly, 1 sub-assembly, 2 part
        if not is_assembly(s):
            return 2
        return 1 if s.asset_information.global_asset_id in child_assets else 0

    shells.sort(key=lambda s: (level(s), (disp_name(s) or "")))

    shells_by_asset = {
        s.asset_information.global_asset_id: s
        for s in shells
        if s.asset_information.global_asset_id
    }
    # asset_id -> EntryNode child Entities (occurrence nodes; OneDown children)
    asm_children_by_asset: dict[str, list] = {}
    for s in shells:
        aid = s.asset_information.global_asset_id
        if not aid:
            continue
        hsm = next(
            (
                ref.resolve(store)
                for ref in s.submodel
                if ref.resolve(store) is not None
                and ref.resolve(store).id_short == "HierarchicalStructures"
            ),
            None,
        )
        if hsm is None:
            continue
        entry = next((e for e in hsm.submodel_element if isinstance(e, model.Entity)), None)
        if entry is None:
            continue
        asm_children_by_asset[aid] = [st for st in entry.statement if isinstance(st, model.Entity)]

    def _entity_kids(entity) -> list:
        return [st for st in getattr(entity, "statement", []) if isinstance(st, model.Entity)]

    def _nav_icon(shell) -> str:
        if is_assembly(shell):
            return "⛓ " if shell.asset_information.global_asset_id not in child_assets else ""
        return "◈ " if anchor(shell.id) in meshes else "· "

    def _nav_link(shell, label: str) -> str:
        return (
            f'<a href="#{anchor(shell.id)}" onclick="event.stopPropagation();'
            f"show3d('{anchor(shell.id)}')\">{_nav_icon(shell)}{esc(label)}</a>"
        )

    def _kids_of(asset_id: str | None, nested: list, trail: frozenset[str]) -> list:
        # Full (flat/single): children already nested under this Entity.
        # OneDown (hierarchical): children live on the referenced assembly AAS.
        if nested:
            return nested
        if asset_id and asset_id not in trail:
            return list(asm_children_by_asset.get(asset_id) or [])
        return []

    def _nav_branch(
        asset_id: str | None, label: str, trail: frozenset[str], nested: list | None = None
    ) -> str:
        nested = nested or []
        shell = shells_by_asset.get(asset_id) if asset_id else None
        shown = label or (disp_name(shell) if shell else "Node")
        aid = (shell.asset_information.global_asset_id if shell else None) or asset_id
        kids = _kids_of(aid, nested, trail)
        if not kids:
            if shell is None:
                return f'<div class="nav-leaf">· {esc(shown)}</div>'
            return f'<div class="nav-leaf">{_nav_link(shell, shown)}</div>'
        next_trail = trail | ({aid} if aid else set())
        inner = "".join(
            _nav_branch(
                getattr(child, "global_asset_id", None),
                _elem_disp(child) or getattr(child, "id_short", None) or "Node",
                next_trail,
                _entity_kids(child),
            )
            for child in kids
        ...
        head = (
            _nav_link(shell, shown)
            if shell is not None
            else f'<span class="nav-orphan">{esc(shown)}</span>'
        )
        return (
            f'<details open class="nav-asm"><summary>{head}</summary>'
            f'<div class="nav-kids">{inner}</div></details>'
        )

    roots = [
        s
        for s in shells
        if is_assembly(s) and s.asset_information.global_asset_id not in child_assets
    ]
    if not roots:
        roots = [s for s in shells if is_assembly(s)] or shells[:1]
    seen: set[str] = set()

    def _mark_entity(entity, trail: frozenset[str]) -> None:
        aid = getattr(entity, "global_asset_id", None)
        next_trail = trail
        if aid:
            if aid in trail:
                return
            seen.add(aid)
            next_trail = trail | {aid}
        for child in _kids_of(aid, _entity_kids(entity), trail):
            _mark_entity(child, next_trail)

    def _mark_shell(shell, trail: frozenset[str]) -> None:
        aid = shell.asset_information.global_asset_id
        if not aid or aid in trail:
            return
        seen.add(aid)
        for child in _kids_of(aid, [], trail):
            _mark_entity(child, trail | {aid})

    for r in roots:
        _mark_shell(r, frozenset())
    leftovers = [
        s
        for s in shells
        if s.asset_information.global_asset_id and s.asset_information.global_asset_id not in seen
    ]
    nav = "".join(
        _nav_branch(r.asset_information.global_asset_id, disp_name(r), frozenset()) for r in roots
    ) + "".join(f'<div class="nav-leaf">{_nav_link(s, disp_name(s))}</div>' for s in leftovers)
...
def render_aas_card(shell, store, files, asset_to_aas, meshes) -> str:
    ...
    btn = (
        f'<button class="btn3d" onclick="show3d(\'{anchor(shell.id)}\')">◈ View 3D</button>'
        if has3d
        else ""
    )
...
<title>__TITLE__ — step2aas viewer</title>
<style>
...
nav{position:sticky;top:64px;flex:0 0 240px;max-height:88vh;overflow:auto;background:var(--card);border:1px solid var(--line);border-radius:10px;padding:8px}
nav a{display:inline-block;max-width:100%;padding:2px 6px;color:var(--fg);text-decoration:none;border-radius:6px;font-size:12.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;vertical-align:middle}
nav a:hover{background:var(--bg)}nav a.sel{background:var(--acc);color:#fff}
nav details.nav-asm{margin:1px 0}nav details.nav-asm>summary{cursor:pointer;list-style:none;padding:1px 0}
nav details.nav-asm>summary::-webkit-details-marker{display:none}
nav details.nav-asm>summary::before{content:"▸ ";color:var(--mut);font-size:11px}
nav details.nav-asm[open]>summary::before{content:"▾ "}
nav .nav-kids{padding-left:12px;border-left:1px solid var(--line);margin:1px 0 2px 8px}
nav .nav-leaf{padding:1px 0}
nav .nav-orphan{display:inline-block;padding:2px 6px;font-size:12.5px}
...
<span style="margin-left:6px">Opacity <input id="opac" type="range" min="0.1" max="1" step="0.05" value="1" oninput="setOpac()"></span>
</div>
<canvas id="glcanvas"></canvas>
<div class="vphint">Drag: rotate · Wheel: zoom · Iso/Front/Top/Right · Fit · Wire · Clip · pick a part in the left list</div></div>
</div>"""
...
if(!m){document.getElementById('vptitle').textContent='(no 3D geometry for this AAS)';document.getElementById('vptris').textContent='';nIdx=0;draw();return;}
...
def open_in_browser(path) -> None:
    ...
    try:
        ...
        logger.warning("Default file opener failed; using browser: %s", exc)
    import webbrowser
...
def main(argv=None) -> int:
    for stream in (sys.stdout, sys.stderr):  # unicode-safe console (cp949 etc.)
        try:
            ...
            logger.debug("Console encoding unchanged: %s", exc)
    p = argparse.ArgumentParser(
        prog="step2aas.viewer",
