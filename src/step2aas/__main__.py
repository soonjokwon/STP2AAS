"""CLI entry (STP2X3D-style flags): python -m step2aas --input <file> [--output <file>]"""
...
def _make_console_unicode_safe() -> None:
    ...
    for stream in (sys.stdout, sys.stderr):
        try:
            ...
            logging.getLogger(__name__).debug("Console encoding unchanged: %s", exc)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(
        ...
        usage="step2aas --input <file> [--output <file.aasx>] [options]",
        epilog=(
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "do not embed the original whole-model file; DigitalFile then holds its path. "
            "Per-part re-exported STEP files are still embedded (they have no external path)."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        "--derive-lightweight",
        action="store_true",
        help="[reserved] derive lightweight geometry (X3D/glTF) — future work §6a",
    )
    p.add_argument(
    ...
    if args.derive_lightweight:
        raise NotImplementedError("Reserved for future work (mapping doc §6a).")
