...
def main() -> int:
    ...
    out_csv = (
        pathlib.Path(sys.argv[3]) if len(sys.argv) > 3 else ROOT / "out" / "fusion_full.csv"
    )
    ...
    for i, archive in enumerate(sorted(arch_dir.glob("*.7z"))):
        ...
        print(f"[extract] {archive.name} -> {chunk_dir.name}: {n} assemblies "
    ...
    fields = [
        "id", "json_bodies", "json_occurrences", "json_components", "step_bytes",
        "t_extract_s", "occurrences", "unique_parts", "dedup_ratio", "is_assembly",
        "t_convert_s", "aasx_bytes", "n_aas", "n_submodels", "n_suppl", "status",
    ]
