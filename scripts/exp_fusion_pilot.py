...
def convert_one(folder: pathlib.Path, out_dir: pathlib.Path) -> dict:
    ...
    from step2aas.aasx_writer import _iter_leaves, _unique_leaf_prototypes, write_aasx
...
def main() -> int:
    ...
    for i, folder in enumerate(assembly_dirs(dataset_dir)):
        ...
        try:
            ...
            failures += 1
            rows.append({"id": folder.name, "status": f"FAIL: {type(exc).__name__}: {exc}"})
            traceback.print_exc()
    ...
    if ok:
        ...
        print(
            f"  occurrences={tot_occ} unique={tot_uni} overall-dedup={tot_occ / max(tot_uni, 1):.2f}"
        )
